function  [MouseLocationCenterOfBody,TimesOfStimuliExploration,TimesInDifferentCompartments,firstFrameInTheAnalysis,LastAnalyzedFrame]=...
   BlackMouseWiredBodyBased4_11_15(filenameBehavioral,HandlesForGUIControls,StartingFrameForAnalysis,EndingFrameForAnalysis,ExcludedAreasList,CompartmentsPositionsList,StimuliPositionsList,SaveMovie,LowThresholdValue, MovieNum);
...
global StopAnalysis
  ...
  for k=StartingFrameForAnalysis:EndingFrameForAnalysis
    ...
    elseif ~isempty(MouseBoundary)
       plot(mean(MouseBoundary(:,2)),mean(MouseBoundary(:,1)),'rX'); 
%        plot(MouseBoundary(:,2),MouseBoundary(:,1),'-r','LineWidth',2) 
