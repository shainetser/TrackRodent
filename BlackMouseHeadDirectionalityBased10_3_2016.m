function  [MouseLocationCenterOfBody,TimesOfStimuliExploration,TimesInDifferentCompartments,firstFrameInTheAnalysis,LastFrameAnalyzed]=...
   BlackMouseHeadDirectionalityBased10_3_2016(filenameBehavioral,HandlesForGUIControls,StartingFrameForAnalysis,EndingFrameForAnalysis,ExcludedAreasList,CompartmentsPositionsList,StimuliPositionsList,SaveMovie,LowThresholdValue,HighThresholdValue, MovieNum);
...
%     plot(MouseEllipseFitParameters.mu_X_fit,MouseEllipseFitParameters.nu_Y_fit,'o','LineWidth',1,'MarkerSize',1);
    ...
    if ~isempty(MouseBoundaryHead)
       if NumOfMousePixelsAroundStimulus1>5
          plot(mean(MouseBoundary_LowThreshold(:,2)),mean(MouseBoundary_LowThreshold(:,1)),'wX',mean(MouseBoundaryHead(:,1)),mean(MouseBoundaryHead(:,2)),'yd');
       ...
       elseif NumOfMousePixelsAroundStimulus3>5
          ...
          plot(mean(MouseBoundary_LowThreshold(:,2)),mean(MouseBoundary_LowThreshold(:,1)),'wX',mean(MouseBoundaryHead(:,1)),mean(MouseBoundaryHead(:,2)),'wd');
       end
    ...
    elseif isempty(MouseBoundary_LowThreshold)  %%%%% activated when the algorithm did not find the mouse
        ...
        plot(mean(MouseBoundary_LowThreshold(:,2)),mean(MouseBoundary_LowThreshold(:,1)),'wX');
    end
